...
import hai.dto.Friendstatus;
import hai.dto.PostAdd;
...
public class FriendController {
    ...
    public String friends(Model model, HttpSession session) {
        ...
        if(loggedInUser.getView()== loggedInUser.getId()){
            return "redirect:/timeline";
        }
        List<Postfindall> postfindalls = postServiceIMPL.getPostsByUserId(loggedInUser.getView());
        ...
        User useview=userServiceIMPL.findById(loggedInUser.getView());
        model.addAttribute("useview",useview);
        int friendstatuslogin= userServiceIMPL.checkFriendship(loggedInUser.getId(), loggedInUser.getView());
        int friendstatusfriend= userServiceIMPL.checkFriendship(loggedInUser.getView(), loggedInUser.getId());
        Friendstatus friendstatus=new Friendstatus();
        if(friendstatuslogin==1&&friendstatusfriend==1){
            friendstatus.setIdsend(-2);
            friendstatus.setStatust("Friend");

        }else if(friendstatuslogin==1&&friendstatusfriend==0) {
            friendstatus.setIdsend(loggedInUser.getId());
            friendstatus.setStatust("Wait");
        } else if(friendstatuslogin==0&&friendstatusfriend==1) {
            friendstatus.setIdsend(loggedInUser.getView());
            friendstatus.setStatust("Confirm");
        }
        else {
            friendstatus.setIdsend(-1);
            friendstatus.setStatust("Add");
        }
    ...
    public String updateAvatar(@RequestParam("newAvatar") MultipartFile newAvatar, HttpSession session) {
        ...
        return "redirect:/friends"; // Chuyển hướng sau khi tải lên thành công
    }
    @GetMapping("/friends/{userId}")
    public String viewFriendDetails(@PathVariable int userId, HttpSession session) {
        User loggedInUser = (User) session.getAttribute("loggedInUser");

        loggedInUser.setView(userId);
        session.setAttribute("loggedInUser",loggedInUser);
    ...
    public String checkfriend(HttpSession session){
        ...
        int friendstatusfriend= userServiceIMPL.checkFriendship(loggedInUser.getView(), loggedInUser.getId());
if(friendstatuslogin==1&&friendstatusfriend==1){
    userServiceIMPL.deleteFriendship(loggedInUser.getId(), loggedInUser.getView());
    userServiceIMPL.deleteFriendship(loggedInUser.getView(),loggedInUser.getId());
} else if(friendstatuslogin==1&&friendstatusfriend==0){
    userServiceIMPL.deleteFriendship(loggedInUser.getId(), loggedInUser.getView());

}
else {
    userServiceIMPL.addFriendship(loggedInUser.getId(),loggedInUser.getView());
} return "redirect:/friends";
    }
}
